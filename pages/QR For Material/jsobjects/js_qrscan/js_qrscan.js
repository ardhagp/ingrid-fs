export default {	
    ...
    await storeValue("QR_Material", {
      ...
      "PO": SplittedText[7],
      "QTY": SplittedText[8],
      "REGID": SplittedText[9],
      "ITEMID": SplittedText[10],
      "BINLOC": SplittedText[11],
      "SPC": SplittedText[12],
...
async f_ExtractString (){
    ...
    if (q_material.data.length === 1) {
      await storeValue("QR_Material2", {
        "MATERIALNAME": q_material.data[0].material_name,
        "PN": q_material.data[0].material_partnumber,
        "UOM": q_material.data[0].uom_code
      });
    }
  	} catch (error) {
    	console.log(error);
